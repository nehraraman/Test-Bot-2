...
# DATABASE_URL will be set via Railway environment variable
DATABASE_URL = os.getenv("DATABASE_URL")
if not DATABASE_URL:
    logger.error("DATABASE_URL environment variable is not set.")
    raise ValueError("DATABASE_URL environment variable is not set.")

ADMIN_IDS = [1756808131]  # Your Telegram ID
ADMIN_USERNAME = "@ramannehra"
BOT_USERNAME = "@X_Reward_Bot"

# Conversation states
(CHANNEL_LINK, SUPPORT_GROUP, TASK_TITLE, TASK_DESCRIPTION, TASK_LINK, TASK_REWARD) = range(6)
...
def init_db():
    try:
        ...
        c.execute('INSERT INTO settings (key, value) VALUES (%s, %s) ON CONFLICT DO NOTHING',
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await check_channel_membership(context, user.id):
        ...
        try:
            ...
            if not c.fetchone():
                c.execute('INSERT INTO users (user_id, username, coins, referrer_id, joined_at) VALUES (%s, %s, %s, %s, %s)',
                ...
                if referrer_id:
                    ...
                    if c.fetchone():
                        c.execute('INSERT INTO referrals (referrer_id, referred_id) VALUES (%s, %s) ON CONFLICT DO NOTHING',
                                  (referrer_id, user.id))
                        c.execute('UPDATE users SET coins = coins + 200 WHERE user_id = %s', (referrer_id,))
                conn.commit()
            keyboard = [
...
async def refer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = user.id
    referral_link = f"https://t.me/{BOT_USERNAME.lstrip('@')}?start={user_id}"
    conn = get_db()
    try:
        c = conn.cursor()
...
async def coins(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        c = conn.cursor()
        c.execute('SELECT coins FROM users WHERE user_id = %s', (user_id,))
        result = c.fetchone()
...
async def daily(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        c = conn.cursor()
        c.execute('SELECT last_claim FROM daily_claims WHERE user_id = %s', (user_id,))
        ...
        if last_claim and (now - datetime.datetime.fromisoformat(last_claim[0])).days < 1:
            ...
            return
        c.execute('UPDATE users SET coins = coins + 50 WHERE user_id = %s', (user_id,))
        c.execute('INSERT INTO daily_claims (user_id, last_claim) VALUES (%s, %s) ON CONFLICT (user_id) DO UPDATE SET last_claim = %s',
...
async def edit_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        c = conn.cursor()
        c.execute('UPDATE tasks SET title = %s, description = %s, link = %s, reward = %s WHERE task_id = %s',
...
async def add_task_reward(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        c = conn.cursor()
        c.execute('INSERT INTO tasks (title, description, link, reward) VALUES (%s, %s, %s, %s)',
    ...
    if data == 'check_join':
        if await check_channel_membership(context, user.id):
            ...
            try:
                ...
                c.execute('SELECT * FROM users WHERE user_id = %s', (user.id,))
                if not c.fetchone():
                    c.execute('INSERT INTO users (user_id, username, coins, joined_at) VALUES (%s, %s, %s, %s)',
...
def main():
    ...
    channel_conv = ConversationHandler(
        ...
        states={CHANNEL_LINK: [MessageHandler(filters.TEXT & ~filters.COMMAND, update_channel_receive)]},
        fallbacks=[CommandHandler('cancel', cancel)]
    )
    support_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(update_support_start, pattern='^update_support$')],
        states={SUPPORT_GROUP: [MessageHandler(filters.TEXT & ~filters.COMMAND, update_support_receive)]},
        fallbacks=[CommandHandler('cancel', cancel)]
    )
